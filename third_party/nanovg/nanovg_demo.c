...
void renderDemo(NVGcontext* vg, float mx, float my, float width, float height, float t, DemoData* data);
void saveScreenShot(int w, int h, const char* name);
...
#define NVG_ASSETS "third_party/nanovg/assets/"
...
int loadDemoData(NVGcontext* vg, DemoData* data)
{
	...
	for (i = 0; i < 12; i++) {
		char file[128];
		snprintf(file, 128, NVG_ASSETS "image%d.jpg", i+1);
	...
	data->fontIcons = nvgCreateFont(vg, "icons", NVG_ASSETS "entypo.ttf");
	if (data->fontIcons == -1) {
		printf("Could not add font icons.\n");
		return -1;
	}
	data->fontNormal = nvgCreateFont(vg, "sans", NVG_ASSETS "Roboto-Regular.ttf");
	if (data->fontNormal == -1) {
		printf("Could not add font italic.\n");
		return -1;
	}
	data->fontBold = nvgCreateFont(vg, "sans-bold", NVG_ASSETS "Roboto-Bold.ttf");
	if (data->fontBold == -1) {
		printf("Could not add font bold.\n");
		return -1;
	}
	data->fontEmoji = nvgCreateFont(vg, "emoji", NVG_ASSETS "NotoEmoji-Regular.ttf");
	if (data->fontEmoji == -1) {
...
int main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		renderDemo(vg, mx,my, winWidth,winHeight, t, &data);
		...
		if (screenshot) {
			screenshot = 0;
			saveScreenShot(fbWidth, fbHeight, "demo1.png");
		}
